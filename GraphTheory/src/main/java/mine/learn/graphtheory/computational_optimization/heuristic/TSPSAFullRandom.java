...
public class TSPSAFullRandom {
     ...
     */
    public TSPSAFullRandom(EdgeWeightedDiGraph graph, int[] set, double t0, double tf, double a, int markov, double p) {
        this.p = p;
        if (a < 0 || a > 1)
    ...
    private void genNewOrder2(int[] order) {
        for (int l = V >> 4; l != V; l++) {// n/2次？
            int i = r.nextInt(V);
            int j = r.nextInt(V);
            while (j == i)
                j = r.nextInt(V);
    ...
    // private int[] shuffle(int[] order) {
    // for (int l = V >> 2; l != V; l++) {// n/2次？
    // int i = r.nextInt(V);
    // int j = r.nextInt(V);
    // while (j == i)
    // j = r.nextInt(V);
    ...
    public static void main(String[] args) throws IOException {
        ...
        // File("src/main/resources/kroB200.json"), best);
        ...
        TSPSAFullRandom tsp = new TSPSAFullRandom(graph, set, 1000, 1, 0.999, 1000, 0.5);
